state.deaths<-function(
	...
	corrected=FALSE,...){
	plot<-plot[1]
	Counts<-read.csv("Weekly_Counts_of_Deaths_by_State_and_Select_Causes__2014-2018.csv")
	Provis<-read.csv("Weekly_Counts_of_Deaths_by_State_and_Select_Causes__2019-2020.csv")
	States<-read.csv("US-population-by-state.csv")
	US.popn<-c(320878310,323015995,325084756,327096265,329064917,331002651)
	State.percent<-setNames(as.numeric(gsub("%","",States$Percent))/100,
	...
	if(plot=="raw"){
		...
		plot(NA,xlim=c(1,52),ylim=c(0,max(Deaths,na.rm=TRUE)),
			bty="n",xlab="",ylab="Death count",...)
		for(i in 1:5) lines(1:52,Deaths[,i],col="grey",...)
		d2020<-Deaths[,"2020"]
		d2020<-d2020[!is.na(d2020)]
		lines(1:(length(d2020)-5),d2020[1:(length(d2020)-5)],lwd=2,
			col=palette()[4],...)
		lines((length(d2020)-5):length(d2020),
			d2020[(length(d2020)-5):length(d2020)],lwd=2,
			col=palette()[4],lty="dashed",...)
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			c("weekly deaths 2015-2019",
			"weekly deaths 2020"),bty="n",cex=0.7,
		...
		mtext(paste("a) total death count (or provisional count), all causes,",
			state),adj=0,line=1,cex=0.8)
		plot(NA,xlim=c(1,52),ylim=c(0,max(colSums(Deaths,na.rm=TRUE))),
			bty="n",xlab="",ylab="Death count",...)
		...
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			...
			"cumulative deaths 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		grid()
		...
		mtext(paste("a) excess death count (or provisional count), all causes,",
			state),adj=0,line=1,cex=0.8)
		plot(NA,xlim=c(1,52),ylim=range(apply(Excess,2,cumsum),na.rm=TRUE),
		...
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			...
			"excess deaths 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		grid()
		mtext(paste(
		...
		par(mfrow=c(2,1),mar=c(5.1,5.1,3.1,2.1))
		plot(NA,xlim=c(1,52),ylim=c(min(PerCapita[,1:5]),
			max(PerCapita,na.rm=TRUE)),bty="n",
		...
		lines(1:(length(pc2020)-5),pc2020[1:(length(pc2020)-5)],lwd=2,
			col=palette()[4],...)
		lines((length(pc2020)-5):length(pc2020),
			pc2020[(length(pc2020)-5):length(pc2020)],lwd=2,
			col=palette()[4],lty="dashed",...)
		legend("topleft","Data in recent weeks\nare incomplete.",lty="dashed",
			lwd=2,col=palette()[4],bty="n",cex=0.7,seg.len=4)
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			c("weekly deaths/1M population 2015-2019",
			"weekly deaths/1M population 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		grid()
		mtext(paste("a) deaths/1M population (or provisional), all causes,",
			state),adj=0,line=1,cex=0.8)
		plot(NA,xlim=c(1,52),ylim=range(apply(PerCapita,2,cumsum),na.rm=TRUE),
		...
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			...
			"cumulative deaths/1M population 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		grid()
		mtext(paste(
		...
		plot(NA,xlim=c(1,52),ylim=c(min(PerCapitaExcess[,1:5]),
			max(PerCapitaExcess,na.rm=TRUE)),bty="n",
		...
		lines(1:(length(pce2020)-5),pce2020[1:(length(pce2020)-5)],lwd=2,
			col=palette()[4],...)
		lines((length(pce2020)-5):length(pce2020),
			pce2020[(length(pce2020)-5):length(pce2020)],lwd=2,
			col=palette()[4],lty="dashed",...)
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			c("excess weekly deaths/1M population 2015-2019",
			"excess weekly deaths/1M population 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		legend("topleft","Data in recent weeks\nare incomplete.",lty="dashed",
			lwd=2,col=palette()[4],bty="n",cex=0.7,seg.len=4)
		...
		mtext(paste("a) excess deaths/1M population (or provisional), all causes,",
			state),adj=0,line=1,cex=0.8)
		plot(NA,xlim=c(1,52),ylim=range(apply(PerCapitaExcess,2,cumsum),na.rm=TRUE),
		...
		legend(x=mean(par()$usr[1:2]),y=par()$usr[3]-0.15*diff(par()$usr[3:4]),
			...
			"cumulative excess deaths/1M population 2020"),bty="n",cex=0.7,
			lwd=c(1,2),col=c("grey",palette()[4]),seg.len=4,xpd=TRUE,
			xjust=0.5,yjust=1)
		grid()
		mtext(paste(
