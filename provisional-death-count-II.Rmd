...
To correct for data incompleteness, the CDC uses a weighting algorithm based on reporting patterns in previous years to correct
estimated weekly totals. From the CDC: *Provisional death counts are weighted to account for incomplete data. However, 
data for the most recent week(s) are still likely to be incomplete. Weights are based on completeness of provisional data in prior years, 
but the timeliness of data may have changed in 2020 relative to prior years, so the resulting weighted estimates may be too high in some 
jurisdictions and too low in others.* 

(I used the *weighted* provisional counts, which are always higher than the *unweighted* raw counts.)

Since the population of the U.S. has been *growing* through time, I also used the following U.S. population size estimates (from 
[worldometers.info](https://www.worldometers.info/world-population/us-population/)) to *correct* expected death counts from 2016-2019 to 
2020 population size. This correction has only a very small effect on the overall estimate of excess death.
...
I would characterize corrected for overall population size *alone* as a very crude approximation, as it does not account for shifting 
demographics through time. In general, the U.S. population getting *older* through time. As such not taking into account shifting 
demographics should tend to *overestimate* the excess mortality of older individuals and *underestimate* the excess mortality of the
very young.

The [dataset](https://www.cdc.gov/nchs/nvss/vsrr/covid19/excess_deaths.htm) that I used for this analysis also included data for 2015.
Including these did not make a large difference, *except* for the analysis of excess deaths in persons aged 25-44 - in which this one 
additional year resulted in a 4% (that is, about a 20% change) in the percent excess death. This is hard to explain, so I decided to 
exclude 2015.
...
The first plot of shows observed weekly deaths in 2020 (grey bars), (corrected) observed weekly deaths from 2016-2019 (black lines), 
and expected weekly deaths (blue line), computed by averaging corrected weekly deaths from 2016-2019.

Data for 2020 are incomplete, and as such have been corrected to account for this incompleteness based on a model of past year reporting
patterns.
...
compared to the (population size corrected) mean for 2016-2019. This is different from the excess mortality reported for government
...
In the following tables, I show total deaths, total deaths per million population, excess deaths, and excess deaths per million 
population. Data for population size by age came from 
[populationpyramid.net](https://www.populationpyramid.net/united-states-of-america/2020/).

The first table gives an analysis of all data through early September, 2020.
